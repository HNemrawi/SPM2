...
def clean_and_standardize_df(
    ...
    existing_cols_to_remove = [
        col for col in cols_to_remove if col in result_df.columns
    ]
...
def create_multiselect_filter(
    ...
    *,
    ...
    module: Optional[str] = None,  # Added for persistence
) -> Optional[List[str]]:
    """
    ...
    if module and key:
        ...
        # Original logic for non-persistent widgets
        if default is None:
            default_list = all_option
        else:
            default_list = [d for d in _to_list(default) if d in choices]
    ...
    try:
        selection = st.multiselect(
            ...
            help=help_text,
            key=key,
            on_change=_on_change_with_persistence if module else on_change,
...
def derive_age_categories(
    ...
    try:
        age_delta = pd.to_datetime(result[ref_date_col]) - pd.to_datetime(
            result[dob_col]
        )
...
def check_date_range_validity(
    ...
    Parameters:
        ...
        df: Optional DataFrame for enhanced warning context
    ...
    if issues:
        ...
        if warn:
            show_universal_date_warning(
                issues, analysis_start, analysis_end, data_start, data_end, df
            )
...
def show_universal_date_warning(
    issues: List[str],
    ...
    """
    Show a universal date range warning that can be used by any filter with dates.
    Uses the UI factory for consistent styling across all modules.

    Parameters:
        issues: List of date range issues
        analysis_start: Start of analysis period
        analysis_end: End of analysis period
        data_start: Start of data period
        data_end: End of data period
        df: Optional DataFrame to check for ReportingPeriod columns
    """
    from src.ui.factories.components import ui

    warning_content_parts = []

    # Add data availability issues
    if issues:
        warning_content_parts.append(
            "<strong>Data Range Issues:</strong><br>"
            "• " + "<br>• ".join(issues)
        )

    # Check against ReportingPeriod bounds if DataFrame is provided
    if df is not None and not df.empty:
        ...
        if (
            ...
            try:
                ...
                if analysis_start < reporting_start:
                    days_before = (reporting_start - analysis_start).days
                    reporting_warnings.append(
                        f"Analysis starts {days_before} days before reporting period"
                    )
                if analysis_end > reporting_end:
                    days_after = (analysis_end - reporting_end).days
                    reporting_warnings.append(
                        f"Analysis ends {days_after} days after reporting period"
                    )
    ...
    if warning_content_parts:
        warning_content = "<br><br>".join(warning_content_parts)
        warning_content += f"<br><br><strong>Selected analysis range:</strong> {analysis_start:%Y-%m-%d} to {analysis_end:%Y-%m-%d}"

        # Determine if data_start/data_end represent ReportingPeriod or Project dates
        if (
            df is not None
            and not df.empty
            and "ReportingPeriodStartDate" in df.columns
            and "ReportingPeriodEndDate" in df.columns
        ):
            try:
                reporting_start = pd.to_datetime(
                ...
                # Check if data_start/data_end match ReportingPeriod dates
                if (
                    data_start.date() == reporting_start.date()
                    and data_end.date() == reporting_end.date()
                ):
                    # data_start/data_end are ReportingPeriod dates
                    warning_content += f"<br><strong>Official reporting period:</strong> {data_start:%Y-%m-%d} to {data_end:%Y-%m-%d}"

                    # Also show actual enrollment data range if available
                    if (
                        "ProjectStart" in df.columns
                        and "ProjectExit" in df.columns
                    ):
                        enrollment_start = pd.to_datetime(
                            df["ProjectStart"].min()
                        )
                        enrollment_end = pd.to_datetime(
                            df["ProjectExit"].max()
                        )
                        warning_content += f"<br><strong>Enrollment data spans:</strong> {enrollment_start:%Y-%m-%d} to {enrollment_end:%Y-%m-%d}"
                else:
                    # data_start/data_end are enrollment dates, show both
                    warning_content += f"<br><strong>Enrollment data range:</strong> {data_start:%Y-%m-%d} to {data_end:%Y-%m-%d}"
                    warning_content += f"<br><strong>Official reporting period:</strong> {reporting_start:%Y-%m-%d} to {reporting_end:%Y-%m-%d}"
            except Exception:
                # Fallback to generic labeling
                warning_content += f"<br><strong>Available data range:</strong> {data_start:%Y-%m-%d} to {data_end:%Y-%m-%d}"
        else:
            # No ReportingPeriod columns available
            warning_content += f"<br><strong>Available data range:</strong> {data_start:%Y-%m-%d} to {data_end:%Y-%m-%d}"
...
def format_percentage(
    value: float, decimals: int = 1, include_sign: bool = False
) -> str:
...
def format_count(value: Union[int, float], decimals: int = 0) -> str:
