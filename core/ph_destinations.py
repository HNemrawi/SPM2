...
def should_apply_custom_destinations() -> bool:
    """Check if custom destinations should be applied based on current analysis module."""
...
def apply_custom_ph_destinations(df: pd.DataFrame, force: bool = False) -> pd.DataFrame:
    """
    ...
    Parameters:
        ...
        force: If True, apply regardless of module (for explicit General Analysis calls)
    """
    ...
    if "selected_ph_destinations" in st.session_state:
        selected_destinations = st.session_state.get("selected_ph_destinations", set())
        
        # Validate selections exist in current data
        available_destinations = set(df["ExitDestination"].dropna().unique())
        selected_destinations = selected_destinations.intersection(available_destinations)
        
        # Update session state with validated selections
        if selected_destinations != st.session_state.get("selected_ph_destinations", set()):
            st.session_state["selected_ph_destinations"] = selected_destinations
        
        # Get original PH destinations for comparison
        ph_mask = df["ExitDestinationCat_Original"] == "Permanent Housing Situations"
        original_ph_destinations = set(df.loc[ph_mask, "ExitDestination"].dropna().unique())
        ...
        if selected_destinations != original_ph_destinations:
            ...
            mask_should_be_ph = df["ExitDestination"].isin(selected_destinations)
            ...
            # Update categories
            df.loc[mask_should_be_ph, "ExitDestinationCat"] = "Permanent Housing Situations"
