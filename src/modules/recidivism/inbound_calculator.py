...
from src.core.constants import PH_CATEGORY
...
def get_last_exit_record(
    row: pd.Series, exits: pd.DataFrame, lookback_days: int
) -> pd.Series:
    """
...
def run_return_analysis(
    ...
    try:
        ...
        entries = (
            ...
            .loc[
                lambda d: (d["ProjectStart"] >= report_start)
                & (d["ProjectStart"] <= report_end)
            ]
        ...
        if allowed_exit_dest_cats and "ExitDestinationCat" in exits:
            exits = exits[
                exits["ExitDestinationCat"].isin(allowed_exit_dest_cats)
            ]

        # Filter by specific exit destinations
        if allowed_exit_destinations and "ExitDestination" in exits:
            exits = exits[
                exits["ExitDestination"].isin(allowed_exit_destinations)
            ]
...
def compute_return_metrics(final_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Specifically count rows labeled as "Returning From Housing (X Days
    # Lookback)"
    returning_housing_count = (
        final_df["ReturnCategory"].str.contains("Returning From Housing").sum()
    )

    # Count rows that say "Returning (X Days Lookback)" but NOT "Returning
    # From Housing"
    returning_only_mask = final_df["ReturnCategory"].str.contains(
        "Returning"
    ...
    pct_returning_housing = (
        (returning_housing_count / total * 100) if total else 0
    )

    # Timing metrics for return entries
    returned_entries = final_df[final_df["ReturnCategory"] != "New"]
    if (
        not returned_entries.empty
        and "days_since_last_exit" in returned_entries.columns
    ):
        valid_times = returned_entries.dropna(subset=["days_since_last_exit"])
        median_days = (
            valid_times["days_since_last_exit"].median()
            if not valid_times.empty
            else 0
        )
        avg_days = (
            valid_times["days_since_last_exit"].mean()
            if not valid_times.empty
            else 0
        )
        max_days = (
            valid_times["days_since_last_exit"].max()
            if not valid_times.empty
            else 0
        )
    else:
...
def return_breakdown_analysis(
    ...
    for group_vals, subdf in grouped:
        ...
        ret = (
            subdf["ReturnCategory"].str.contains("Returning")
            & ~subdf["ReturnCategory"].str.contains("From Housing")
        ).sum()
        ret_housing = (
            subdf["ReturnCategory"]
            .str.contains("Returning From Housing")
            .sum()
        )

        valid_time = subdf.dropna(subset=["days_since_last_exit"])
        median_days = (
            valid_time["days_since_last_exit"].median()
            if not valid_time.empty
            else 0
        )
        avg_days = (
            valid_time["days_since_last_exit"].mean()
            if not valid_time.empty
            else 0
        )
