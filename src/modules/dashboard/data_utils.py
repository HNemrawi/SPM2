...
from src.core.constants import (
    NON_HOMELESS_PROJECTS as NON_HOMELESS_SERVICES_PROJECTS,
)
...
class ProjectTypes:
    ...
    PERMANENT_HOUSING: Set[str] = PERMANENT_HOUSING_PROJECTS

    NON_HOMELESS_SERVICES: Set[str] = NON_HOMELESS_SERVICES_PROJECTS
...
def calculate_change(
    ...
    try:
        ...
        pct_change = safe_divide(
            absolute_change, abs(previous), multiplier=100
        )
...
class ReturnToHomelessness:
    ...
    def identify_returners(
        ...
        required_full_cols = REQUIRED_BASE_COLS + [
            "ProjectTypeCode",
            "HouseholdMoveInDate",
            "EnrollmentID",
        ]
        ...
        ph_exits = df_filtered[
            ...
            & (
                df_filtered["ExitDestinationCat"]
    ...
    def _check_return(
        ...
        for _, enrollment in next_enrollments.iterrows():
            ...
            if is_ph:
                ...
                if (
                    pd.notna(enrollment.get("HouseholdMoveInDate"))
                    and enrollment["ProjectStart"]
                    == enrollment["HouseholdMoveInDate"]
                ):
                    continue

                # Handle short stays (≤14 days)
                if gap_days <= 14:
                    if pd.notna(enrollment.get("ProjectExit")):
                        exclusion_windows = (
                            ReturnToHomelessness._merge_windows(
                                (
                                    enrollment["ProjectStart"]
                                    + pd.Timedelta(days=1),
                                    enrollment["ProjectExit"]
                ...
                if any(
                    start <= enrollment["ProjectStart"] <= end
...
def category_counts(
    ...
    return (
        filtered_df.groupby(group_column, observed=True)["ClientID"]
        .nunique()
        .rename(series_name)
...
class TimeSeriesAnalysis:
    ...
    def calculate_metric_series(
        ...
        for period in periods:
            ...
            results.append({"bucket": period_start, "count": len(client_ids)})
    ...
    def calculate_metric_series_by_group(
        ...
        for group_value, group_df in df.groupby(group_column, dropna=True):
            for period in periods:
                ...
                client_ids = metric_function(
                    group_df, period_start, period_end
                )
                results.append(
                    {
                        "bucket": period_start,
    ...
    def calculate_growth_rates(df: DataFrame) -> DataFrame:
        ...
        try:
            ...
            growth_df = pd.merge(
                first_data, last_data, on="group", suffixes=("_first", "_last")
            )
            ...
            growth_df["growth"] = (
                growth_df["last_count"] - growth_df["first_count"]
            )
            ...
            return growth_df[
                ["group", "first_count", "last_count", "growth", "growth_pct"]
...
recalculated_metric_time_series_by_group = (
    TimeSeriesAnalysis.calculate_metric_series_by_group
)
