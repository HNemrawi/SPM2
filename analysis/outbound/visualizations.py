...
def display_spm_metrics(metrics: Dict[str, Any]) -> None:
    """
    Display outbound recidivism metrics as cards with natural theming.
    ...
    col3.metric("Total Returns", f"{metrics['Return']:,}")
    ...
    col7.metric("Median Days", f"{metrics['Median Days']:.1f}", help="Median days to ANY return enrollment")
    col8.metric("Average Days", f"{metrics['Average Days']:.1f}", help="Average days to ANY return enrollment")
    col9.metric("Max Days", f"{metrics['Max Days']:.0f}", help="Maximum days to ANY return enrollment")
...
def display_spm_metrics_ph(metrics: Dict[str, Any]) -> None:
    ...
    col2.metric("Total Returns", f"{metrics['Return']:,}")
...
def display_spm_metrics_non_ph(metrics: Dict[str, Any]) -> None:
    ...
    style_metric_cards(
        border_left_color=NeutralColors.WARNING,
        box_shadow=True
    )
    ...
    flows = [
        {
            ...
            "Return": tgt,
            "Count": int(val),
            "Percent": round((val / total * 100) if total else 0, 1),
        }
...
def plot_flow_sankey(
    ...
    node_colors = (
        ...
        [NeutralColors.CHART_COLORS[1]] * len(return_cats)
    )
