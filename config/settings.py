...
class PageConfig:
    ...
    title: str = "Homelessness Analysis Suite"
    icon: str = "📊"
...
class DateConfig:
    ...
    default_start_date: str = field(default_factory=lambda: (datetime.now() - timedelta(days=31)).strftime("%Y-%m-%d"))
    default_end_date: str = field(default_factory=lambda: (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d"))
    
    # Date constraints
    min_date: str = "2000-01-01"
    max_date: str = field(default_factory=lambda: datetime.now().strftime("%Y-%m-%d"))
    ...
    # Common time periods
    quick_periods: Dict[str, int] = field(default_factory=lambda: {
        "Last 30 days": 30,
        "Last 90 days": 90,
        "Last 6 months": 180,
        "Last year": 365,
...
class DataConfig:
    ...
    encoding_options: List[str] = field(default_factory=lambda: ["utf-8", "latin-1", "cp1252"])
    ...
    date_parse_formats: List[str] = field(default_factory=lambda: [
        "%Y-%m-%d",
        "%m/%d/%Y",
        "%d/%m/%Y",
        "%Y/%m/%d",
...
class AnalysisConfig:
    ...
    # Default filters
    default_project_types: List[str] = field(default_factory=lambda: [
        "Emergency Shelter",
    ...
    demographic_groups: Dict[str, List[str]] = field(default_factory=lambda: {
...
class UIConfig:
    ...
    chart_colors: List[str] = field(default_factory=lambda: [
        "#0066CC", "#059862", "#D97706", "#DC2626",
        "#7C3AED", "#0891B2", "#EC4899", "#6366F1"
    ])
    
    # Messages
    loading_messages: List[str] = field(default_factory=lambda: [
        "Analyzing your data...",
    ...
    def override_from_env(cls, config: Any) -> Any:
        ...
        for field_name in config.__dataclass_fields__:
            ...
            if env_key in os.environ:
                ...
                setattr(config, field_name, cls.get_env_var(env_key, var_type=field_type))
...
class AppConfig:
    ...
    def _validate(self):
        ...
        self.analysis.significance_level = max(0.001, min(0.5, self.analysis.significance_level))
...
def format_date(date: Union[str, datetime], format_type: str = "display") -> str:
...
def is_development() -> bool:
    ...
    return EnvironmentConfig.get_env_var("HMIS_ENV", "production") == "development"
